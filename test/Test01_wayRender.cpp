...
int main()
{
    ...
    std::vector<unsigned int> HeightWayIndices;
    int wayCnt = 0;
    int buildingCnt = 0;
    ...
    for (auto& way : OsmData.Ways)
    {
        ...
        bool IsRailWay = way.Tags.count("cycleway") && way.Tags.count("name:en");
        bool IsHeighWay = way.Tags.count("highway");
        //if (IsRailWay) {
        ...
        for (int i = 1; i < way.NodeIds.size(); ++i)
        {
            ...
            if (IsHeighWay)
            {
                HeightWayIndices.push_back(Index0);
                HeightWayIndices.push_back(Index1);
            }
            ...
            if (IsRailWay)
            {
                if (1 == i)
                {
                    RailwayIndices.push_back(RaiLwayVertices.size());
                    RaiLwayVertices.emplace_back(Vertices[Index0]);
                }
                RailwayIndices.push_back(RaiLwayVertices.size());
                RaiLwayVertices.emplace_back(Vertices[Index1]);
            }
        }
    ...
    IndexBuffer* HeighWayIbPtr = new IndexBuffer(HeightWayIndices.data(), HeightWayIndices.size());
    VertexArray* BarVaPtr = new VertexArray(AllVerticesVbPtr, HeighWayIbPtr);
    HeighWayIbPtr->Bind();
    ...
    // 绘制地铁
    VertexBuffer* RailwayVbPtr = new VertexBuffer(RaiLwayVertices.data(), RaiLwayVertices.size() * sizeof(RaiLwayVertices[0]));
    RailwayVbPtr->Push<float>(3, false);
    RailwayVbPtr->Push<float>(3, false);
    IndexBuffer* RailwayIbPtr = new IndexBuffer(RailwayIndices.data(), RailwayIndices.size());
    VertexArray* RailwayVaPtr = new VertexArray(RailwayVbPtr, RailwayIbPtr);
    Shader* RailwayShader = new Shader("shader/railway/railway.vert", "shader/railway/railway.frag");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // 绘制地铁
        //RailwayShader->Use();
        //RailwayShader->setView(camera, window);
        //RailwayShader->SetUniformMat4(TIModel, "TIModel");
        //RailwayVaPtr->Bind();
        //RailwayIbPtr->Bind();
        ////RailwayVaPtr->DrawArray(*RailwayShader);

        //GLCall(glDrawElements(GL_LINES, RailwayIbPtr->GetCount(), GL_UNSIGNED_INT, nullptr));
        ...
        BarVaPtr->Bind();
        HeighWayIbPtr->Bind();
        GLCall(glDrawElements(GL_LINES, HeighWayIbPtr->GetCount(), GL_UNSIGNED_INT, nullptr));
