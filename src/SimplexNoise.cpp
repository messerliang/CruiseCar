...
float FbmPerlin3D::hash1(glm::vec3 p)
{
    p = glm::fract(p * glm::vec3(0.3183099, 0.3678794, 0.7071067) + 0.1f);
    p *= 17.0;
    return glm::fract(p.x * p.y * p.z * (p.x + p.y + p.z));
}

glm::vec3 FbmPerlin3D::grad3(glm::vec3 p)
{
    // 
    float h1 = hash1(p + glm::vec3(0.0, 0.0, 0.0));
    float h2 = hash1(p + glm::vec3(1.3, 7.1, 2.4));
    float h3 = hash1(p + glm::vec3(8.2, 3.7, 5.6));

    glm::vec3 g = glm::normalize(glm::vec3(h1 * 2.0 - 1.0, h2 * 2.0 - 1.0, h3 * 2.0 - 1.0));

    return g;
}

glm::vec3 FbmPerlin3D::fade(glm::vec3 t) {
    return t * t * t * (t * (t * 6.0f - 15.0f) + 10.0f);
}

float FbmPerlin3D::perlin3(glm::vec3 p) {
    // lattice cell coordinates and local (0..1) position
    glm::vec3 Pi = glm::floor(p);
    glm::vec3 Pf = glm::fract(p);
    ...
    // corners: (0,0,0) .. (1,1,1)
    float n000 = glm::dot(grad3(Pi + glm::vec3(0.0, 0.0, 0.0)), Pf - glm::vec3(0.0, 0.0, 0.0));
    float n100 = glm::dot(grad3(Pi + glm::vec3(1.0, 0.0, 0.0)), Pf - glm::vec3(1.0, 0.0, 0.0));
    float n010 = glm::dot(grad3(Pi + glm::vec3(0.0, 1.0, 0.0)), Pf - glm::vec3(0.0, 1.0, 0.0));
    float n110 = glm::dot(grad3(Pi + glm::vec3(1.0, 1.0, 0.0)), Pf - glm::vec3(1.0, 1.0, 0.0));
    float n001 = glm::dot(grad3(Pi + glm::vec3(0.0, 0.0, 1.0)), Pf - glm::vec3(0.0, 0.0, 1.0));
    float n101 = glm::dot(grad3(Pi + glm::vec3(1.0, 0.0, 1.0)), Pf - glm::vec3(1.0, 0.0, 1.0));
    float n011 = glm::dot(grad3(Pi + glm::vec3(0.0, 1.0, 1.0)), Pf - glm::vec3(0.0, 1.0, 1.0));
    float n111 = glm::dot(grad3(Pi + glm::vec3(1.0, 1.0, 1.0)), Pf - glm::vec3(1.0, 1.0, 1.0));

    // trilinear interpolation using fade weights
    float nx00 = glm::mix(n000, n100, w.x);
    float nx10 = glm::mix(n010, n110, w.x);
    float nx01 = glm::mix(n001, n101, w.x);
    float nx11 = glm::mix(n011, n111, w.x);

    float nxy0 = glm::mix(nx00, nx10, w.y);
    float nxy1 = glm::mix(nx01, nx11, w.y);

    float nxyz = glm::mix(nxy0, nxy1, w.z);

    // nxyz is roughly in [-sqrt(3)/2, sqrt(3)/2], scale to [-1,1] comfortably
    return glm::clamp(nxyz * 1.6, 0.0, 1.0);
}

float FbmPerlin3D::fbmPerlin3(glm::vec3 p, int octaves, float lacunarity, float gain) {
    float amp = 1.0;
    float freq = 1.0;
    float sum = 0.0;
    float maxAmp = 0.0;
